...
// ReSharper disable AccessToDisposedClosure
		...
		public static List<TokeiTuple<string, string>> ConvertToSegments(IProgress container, string fileName) {
			List<TokeiTuple<string, string>> filesGrf = new List<TokeiTuple<string, string>>();
			string decompressedFileName = Path.Combine(Settings.TempPath, Path.GetRandomFileName());
			...
			try {
				...
				while (offset <= arrDecompressed.Length) {
					...
					if (entryType == 'f') {
						...
						filesGrf.Add(new TokeiTuple<string, string>(name, path));
					}
					else if (entryType == 'd') {
					}
					else if (entryType == 'e')
