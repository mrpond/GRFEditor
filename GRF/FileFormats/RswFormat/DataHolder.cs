...
namespace GRF.FileFormats.RswFormat {
	public sealed class DataHolder {
		public static List<TokeiTuple<int, int, List<QuadTreeNode>>> QuadTrees = new List<TokeiTuple<int, int, List<QuadTreeNode>>>();
		...
		public static void AddQuadTree(int sizeX, int sizeY, List<QuadTreeNode> nodes) {
			QuadTrees.Add(new TokeiTuple<int, int, List<QuadTreeNode>>(sizeX, sizeY, nodes));
		}
	}
}