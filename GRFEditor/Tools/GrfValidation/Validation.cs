...
namespace GRFEditor.Tools.GrfValidation {
	public class Validation : IProgress {
		...
		public void FindErrors(List<TokeiTuple<ValidationTypes, string, string>> errors, GrfHolder container) {
			try {
				_initProgress();
				errors.Clear();
				...
				if (GrfEditorConfiguration.FeSpaceSaved) {
					...
					if (size > 0) {
						...
						errors.Add(new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeSpaceSaved, "", spaceSavedString));
					}
				}
				...
				if (GrfEditorConfiguration.FeInvalidFileTable) {
					...
					errors.AddRange(entriesList.Where(p => p.SizeCompressedAlignment < p.SizeCompressed)
						                .Select(p => new TokeiTuple<ValidationTypes, string, string>(
				...
				if (GrfEditorConfiguration.FeNoExtension) {
					errors.AddRange(entries.Where(p => !Path.GetFileName(p.Key).
						                                    Contains('.')).Select(p => new TokeiTuple<ValidationTypes, string, string>(
				...
				if (GrfEditorConfiguration.FeMissingSprAct) {
					...
					errors.AddRange(
						...
						select new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeMissingSprAct, act.RelativePath, "Complementing Spr file missing."));

					errors.AddRange(
						from spr in sprFiles
						where !actFileNamesCut.Contains(spr.RelativePath.Replace(Path.GetExtension(spr.RelativePath), ""))
						select new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeMissingSprAct, spr.RelativePath, "Spr file is not used."));
				}
				...
				if (GrfEditorConfiguration.FeEmptyFiles) {
					...
					errors.AddRange(entries.Where(p => p.Value.DisplaySize == emptySizeString).Select(p =>
					                                                                                  new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeEmptyFiles, p.Key, "Empty file (size = 0), remove it.")));
				}
				...
				if (GrfEditorConfiguration.FeDb) {
					errors.AddRange(entries.Where(p => Path.GetExtension(p.Key) != null && Path.GetExtension(p.Key).ToLower() == ".db").Select(p =>
					                                                                                                                           new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeDb, p.Key, "Hidden database file for thumbnails, remove it.")));
				}
				...
				if (GrfEditorConfiguration.FeSvn) {
					errors.AddRange(entries.Where(p => Path.GetExtension(p.Key) != null && Path.GetExtension(p.Key).ToLower() == ".svn").Select(p =>
					                                                                                                                            new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeSvn, p.Key, "Subversion file (usually hidden), remove it.")));
				}
				...
				if (GrfEditorConfiguration.FeDuplicateFiles) {
					...
					errors.AddRange(entriesDuplicates.Where(p => p.Value > 0).Select(p =>
					                                                                 new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeDuplicateFiles, p.Key, "This file has been found " + (p.Value + 1) + " times.")));
				}

				if (GrfEditorConfiguration.FeRootFiles) {
					foreach (KeyValuePair<string, FileEntry> entry in entries) {
						if (String.IsNullOrEmpty(Path.GetDirectoryName(entry.Key))) {
							errors.Add(new TokeiTuple<ValidationTypes, string, string>(
				...
				if (GrfEditorConfiguration.FeDuplicatePaths) {
					...
					errors.AddRange(entriesDuplicates.Where(p => p.Value > 0).Select(p =>
					                                                                 new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.FeDuplicatePaths, p.Key, "This path has been found " + (p.Value + 1) + " times.")));
				}
			}
			catch (Exception err) {
		...
		public void ValidateContent(List<TokeiTuple<ValidationTypes, string, string>> errors, GrfHolder container, ref MultiGrfReader metaGrf) {
			try {
				_initProgress();
				errors.Clear();
                ...
                Parallel.For(0, entries.Count, index => {
					...
					try {
													...
													if (GrfEditorConfiguration.VcSpriteIssues) {
														if (GrfEditorConfiguration.VcSpriteIssuesRle) {
															if (spr.GetEarlyEndingEncoding().Cast<object>().Any()) {
																errors.Add(new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.VcSpriteIssuesRle, entry.RelativePath, "Early ending RLE encoding (usually not an issue)."));
															}
														}
													}
													break;
						...
						//_failedLoading(errors, entry);
						errors.Add(new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.VcUnknown, entry.RelativePath, err.Message));
					}
		...
		private void _validateAct(Act act, List<TokeiTuple<ValidationTypes, string, string>> errors, FileEntry entry, MultiGrfReader metaGrfLocalCopy) {
			if (GrfEditorConfiguration.VcSpriteIssues) {
					...
					for (int j = 0; j < act[i].NumberOfFrames; j++) {
						...
						if (GrfEditorConfiguration.VcSpriteSoundIndex) {
							if (frame.SoundId >= act.SoundFiles.Count) {
								errors.Add(new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.VcSpriteSoundIndex,
						...
						if (GrfEditorConfiguration.VcSpriteIndex) {
							for (int k = 0; k < frame.NumberOfLayers; k++) {
								if (frame.Layers[k].SpriteIndex >= act.Sprite.NumberOfImagesLoaded) {
									errors.Add(new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.VcSpriteIndex,
		...
		private void _validatePaths(MultiGrfReader metaGrf, IEnumerable<string> paths, ValidationTypes type, string file, string error, List<TokeiTuple<ValidationTypes, string, string>> errors) {
			foreach (string path in paths) {
				if (!metaGrf.Exists(path)) {
					errors.Add(new TokeiTuple<ValidationTypes, string, string>(type, file, String.Format(error, path)));
				}
			}
		}

		private void _failedLoading(List<TokeiTuple<ValidationTypes, string, string>> errors, FileEntry entry) {
			errors.Add(new TokeiTuple<ValidationTypes, string, string>(ValidationTypes.VcLoadEntries, entry.RelativePath, "Failed to load file's content (corrupted?)."));
		}
		...
		public void ValidateExtraction(List<TokeiTuple<ValidationTypes, string, string>> errors, GrfHolder grfHolder,
		...
		public void ComputeHash(List<TokeiTuple<ValidationTypes, string, string>> errors, GrfHolder grfHolder, IHash hash, bool direction, string path) {
			try {
				_initProgress();
				errors.Clear();
		...
		public void ComputeHashQuick(List<TokeiTuple<TkPath, byte[]>> errors, GrfHolder grfHolder, IHash hash) {
			try {
				_initProgress();
				errors.Clear();
                ...
                Parallel.For(0, entries.Count, index => {
					try {
						if (!IsCancelling) {
							...
							lock (_lock) {
								errors.Add(new TokeiTuple<TkPath, byte[]>(
